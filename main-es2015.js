...
let HttpServiceHelper = class HttpServiceHelper {
    constructor(http) {
        this.http = http;
    }
    httpGetRequest(url) {
        return this.http.get(url)
            ...
            .catch(response => (rxjs_Rx__WEBPACK_IMPORTED_MODULE_1__["Observable"].throw(response)));
    }
};
HttpServiceHelper.ctorParameters = () => [
...
let DealDocumentComponent = class DealDocumentComponent {
    constructor(_Activatedroute, _router, _dealService) {
...
DealDocumentComponent.ctorParameters = () => [
...
let TodoListComponent = class TodoListComponent {
    constructor(todoListService, broadcastService, msalService) {
    ...
    ngOnInit() {
        ...
        this.subscription = this.broadcastService.subscribe("msal:acquireTokenFailure", (payload) => {
            ...
            if (payload.errorDesc.indexOf("consent_required") !== -1 || payload.errorDesc.indexOf("interaction_required") != -1) {
                this.msalService.acquireTokenPopup(_common_app_config__WEBPACK_IMPORTED_MODULE_4__["AppConfig"].webApiScopes).then((token) => {
...
let TodoListService = class TodoListService {
    ...
    getItems() {
        return this.http.get(_common_app_config__WEBPACK_IMPORTED_MODULE_4__["AppConfig"].webApiEndpoint + "/deals/hardcoded")
            ...
            .catch(response => (rxjs_Rx__WEBPACK_IMPORTED_MODULE_3__["Observable"].throw(response)));
    }
    getSPItems() {
    ...
    postFile(fileToUpload, id) {
        const endpoint = _common_app_config__WEBPACK_IMPORTED_MODULE_4__["AppConfig"].webApiEndpoint + "/deals/documents/" + id + "/upload";
...
let SharepointDataComponent = class SharepointDataComponent {
    constructor(spListService, broadcastService, msalService) {
    ...
    ngOnInit() {
        ...
        this.subscription = this.broadcastService.subscribe("msal:acquireTokenFailure", (payload) => {
            console.log("acquire token failure " + JSON.stringify(payload));
            if (payload.errorDesc.indexOf("consent_required") !== -1 || payload.errorDesc.indexOf("interaction_required") != -1) {
                this.msalService.acquireTokenPopup(_common_app_config__WEBPACK_IMPORTED_MODULE_4__["AppConfig"].spApiScopes).then((token) => {
...
SharepointDataComponent.ctorParameters = () => [
...
let SPListService = class SPListService {
    constructor(http) {
        this.http = http;
    }
    getItems() {
...
let SPapiComponent = class SPapiComponent {
    constructor(todoListService, broadcastService, msalService) {
        this.todoListService = todoListService;
        this.broadcastService = broadcastService;
        this.msalService = msalService;
...
let UserDataComponent = class UserDataComponent {
    constructor(authService, http, httpService, broadcastService) {
        this.authService = authService;
        this.http = http;
